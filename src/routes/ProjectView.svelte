...
<div class="w-full grid grid-rows-6 grid-cols-3 md:grid-cols-6 md:grid-rows-3 max-sm:h-[70rem] sm:h-[24rem] md:h-[30rem] lg:h-[52rem] align-top">
{#each relevantProjects as project (project.url)}
    <button animate:flip={{duration:400}} transition:fade on:click={() => {chosenProject.set(project)}} class="rounded-xl w-full text-center text-xl max-sm:px-2 sm:px-5  sm:py-5">
